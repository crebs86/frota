...
function buscarCota() {
    ...
    if (!(busca.value.posto_coleta || busca.value.contrato)) {
        toast.error('Selecione um Posto de Coleta ou um Contrato')
    } else {
...
function salvarCota() {
    if (!cota.value.valor || !cota.value.inicio || !cota.value.fim || !busca.value.posto_coleta || !busca.value.contrato) {
        ...
        axios.post(route('regulacao.financeiro.cota.salvar-cota'), {
            contrato: busca.value.contrato.hash,
            posto_coleta: busca.value.posto_coleta.hash,
            inicio: cota.value.inicio,
            fim: cota.value.fim,
            valor: cota.value.valor,
        })
            .then((r) => {
                        ...
                        <div class="grid grid-cols-3 gap-3 place-items-center">
                            ...
                            <form @submit.prevent="buscarCota()" class="col-span-3 w-full">
                                <div class="col-span-3 w-full">
                                    ...
                                    <Select id="posto" v-model="busca.posto_coleta" :options="props.postos_coleta"
                                            placeholder="Selecione um Posto de Coleta ou" class="w-full" filter
                                ...
                                <div class="col-span-4 inline-flex gap-3">
                                    <Button type="submit" class="mt-3" severity="info" :disabled="busca.loading">
                                        Buscar Cota
                                    </Button>
                                    <Button type="button" class="mt-3" severity="warn"
                                            @click="limpar(); cotaUnidade = []; busca.criarCota = false">
                                        Limpar
                                    </Button>
                                </div>
                            ...
                            <div class="col-span-3 w-full" v-if="cotaUnidade.length > 0">
                                <DataTable :value="cotaUnidade" table-style="width: 100%; min-width: 60rem"
                                           class="mt-2">
                                    <Column field="name" header="Posto de Coleta" style="width: 20%"/>
                                    <Column field="valor" header="Valor Atual" style="width: 12%">
                                        <template #body="slotProps">
                                            R$ {{ currencyMask(slotProps.data.valor) }}
                                        </template>
                                    </Column>
                                    <Column field="n_contrato" header="Contrato/Ano" style="width: 15%">
                                        <template #body="slotProps">
                                            {{ slotProps.data.n_contrato }}
                                            /{{ slotProps.data.ano }}
                                        </template>
                                    </Column>
                                    ...
                                    <Column field="inicio" header="Início" style="width: 10%">
                                        <template #body="slotProps">
                                            {{ moment(slotProps.data.inicio).format('DD/MM/YYYY') }}
                                        </template>
                                    </Column>
                                    <Column field="fim" header="Fim" style="width: 10%">
                                        <template #body="slotProps">
                                            {{ moment(slotProps.data.fim).format('DD/MM/YYYY') }}
                                        </template>
                                    </Column>
                                    <Column field="id" header="Ações" style="width: 8%">
                                        <template #body="slotProps">
                                ...
                                <form @submit.prevent="atualizarCota()">
                                    <ul>Detalhes
                                        <li><span class="font-bold">Contrato: </span>
                                            {{ cotaAtual.n_contrato }}/{{ cotaAtual.ano }}
                                        </li>
